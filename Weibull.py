...
logger = logging.getLogger('weibull')  

# set log level
logger.setLevel(logging.INFO)

# define file handler and set formatter
file_handler = logging.FileHandler('30_wb_50000_3000_0.5.log')
...
for i in range(0, 1000):
        #dat = aft_sim.aft_simulation(n = 800, p = 5000, r = 0.5, 
        #                b0 = 0, b1 = np.concatenate((np.array([0.001]*2500), np.array([0]*2500))), sig = 2,
        #                test_set = 0.25)
        ...
        df_train, df_test, a_tr, a_tt = wb(n = 50000, p = 3000, r = 0.5, 
                        b0 = 0, b1 = np.concatenate((np.array([0.01]*1500), np.array([0]*1500))), sig = 2,
                        test_set = 0.25, c_percentage = 70, c_lower = 26, c_higher = 34)
        ...
        acc_tr_lgb.append(surv_median(true = y_true, pred = np.exp(y_pred)))
        acc_tt_lgb.append(surv_median(true = st_yc_tt, pred = st_aft_lgb.predict(X = st_X_tt)))
...
pd.DataFrame({'acc_tr_lgb': acc_tr_lgb,
             ...
             'c_tt' : c_tt,
             'time_lgb': time_lgb,
             'time_xgb': time_xgb}).to_pickle('30_wb_simulation_50000_3000_0.5.pkl')
...
# define file handler and set formatter
file_handler = logging.FileHandler('30_wb_50000_3000_0.log')
formatter    = logging.Formatter('%(asctime)s : %(levelname)s : %(name)s : %(message)s')
file_handler.setFormatter(formatter)
...
pd.DataFrame({'acc_tr_lgb': acc_tr_lgb,
             ...
             'c_tt' : c_tt,
             'time_lgb': time_lgb,
             'time_xgb': time_xgb}).to_pickle('30_wb_simulation_50000_3000_0.pkl')
